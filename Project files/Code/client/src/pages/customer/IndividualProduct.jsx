...
useEffect(()=>{
    fetchProduct();
    // Refresh userId from localStorage
...
const fetchProduct = async () =>{
    await axios.get(`http://localhost:6001/fetch-product-details/${id}`).then(
        (response)=>{
            ...
            console.log('Product loaded - sizes available:', response.data.sizes);
        }
    )
}
...
const handleAddToCart = async() =>{
    // Refresh userId from localStorage before checking
    const currentUserId = localStorage.getItem('userId');
    setUserId(currentUserId);
    
    console.log('Add to cart - userId:', currentUserId, 'size:', size);
    ...
    await axios.post('http://localhost:6001/cart/add', {
        ...
        <div className="IndividualProduct-body">
            ...
            <div className="IndividualProduct-data">
                ...
                <span>
                    ...
                    <select name="productSize" id="productSize" value={size} onChange={(e)=>{
                    ...
                    {size && <p style={{color: 'green', fontSize: '12px'}}>Selected size: {size}</p>}
                </span>
                <span>
